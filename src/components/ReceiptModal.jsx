        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl bg-emerald-50 dark:bg-slate-800 p-5">
            ...
            <div className="mt-3 space-y-1 text-sm text-slate-700 dark:text-slate-300">
              <p>{order.customerDetails?.fullName || 'Marketplace customer'}</p>
              <p>{order.customerDetails?.phoneNumber || 'Phone unavailable'}</p>
              <p>{order.customerDetails?.emailAddress || 'Email unavailable'}</p>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-emerald-50 dark:bg-slate-800 p-5">
            ...
            <div className="mt-3 space-y-1 text-sm text-slate-700 dark:text-slate-300">
              <p>{order.customerDetails?.streetAddress || 'Address unavailable'}</p>
              <p>
                {[order.customerDetails?.city, order.customerDetails?.postalCode]
                  .filter(Boolean)
                  .join(', ') || 'City and postal code unavailable'}
              </p>
              <p>{order.eta}</p>
            </div>
          </div>
